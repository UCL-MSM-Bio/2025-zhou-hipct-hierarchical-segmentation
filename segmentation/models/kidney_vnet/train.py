...
def validation_for_epoch(epoch, net, metricer, val_loader, loss_func):
    val_dices = []
    mean_loss = []
    # validation
    with torch.no_grad(): #
        for step, (image, label) in enumerate(val_loader):
            ...
            prediction = net(image)  # [b,2,48,384,240]
            #label = label.unsqueeze_(1) #[b,1,48,384,240]
            ...
            prediction_arg = prediction.argmax(dim=1).float()  # [b,48,384,240]
    ...
    metricer.reset()
    print('val_loss:', val_epoch_loss, 'val dice: ', val_epoch_dice)
...
if __name__ == '__main__':
    ...
    data_train = KidneyData(root=data_path, mode='train', separation=config.SEPARATION, fold=config.FOLD, all_zero_involved=False, transforms=train_transforms)  # image : [batch, 1, 48, 384, 240]) label: [batch, 48, 384, 240]
    ...
    #loss_func = TverskyLoss(include_background=False, to_onehot_y=True, beta=0.7, alpha=0.3)
    ...
    for epoch in range(start_epoch, config.NUM_EPOCHS):
        ...
        net.eval()
        val_loss, val_dice = validation_for_epoch(epoch, net, metricer, val_loader,loss_func)
    ...
    torch.save(state, os.path.join(config.SAVE_PATH, config.SAVE_MODEL_NAME+'_final.tar'))
